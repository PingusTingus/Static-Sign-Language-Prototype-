...
try:
    ...
    RPI_AVAILABLE = False
    print("Warning: RPi.GPIO not available. Running in standard mode.")
...
# Setup for Raspberry Pi GPU if available
try:
    os.environ['OPENCV_OPENCL_RUNTIME'] = ''  # Try to enable OpenCL for CV
    os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = 'true'  # Allow TF to allocate GPU memory as needed
except:
    pass
...
STATIONARY_THRESHOLD = 5  # Reduced from 7 to 5 for Raspberry Pi responsiveness
...
# Check if Tensorflow Lite Interpreter is available (better for Raspberry Pi)
try:
    tf.config.threading.set_intra_op_parallelism_threads(2)  # Limit TF threads
    tf.config.threading.set_inter_op_parallelism_threads(1)

    # Try to use the TPU if available
...
print(f"Running on Raspberry Pi: {RPI_AVAILABLE}")
...
class TextToSpeech:
    ...
    def __init__(self):
        ...
        try:
            ...
            # Set properties - lower values for Raspberry Pi
...
class KeyPointClassifier(object):
    def __init__(self, model_path=KEYPOINT_CLASSIFIER_PATH, num_threads=1):
        ...
        # For Raspberry Pi, use 2 threads for better performance
        threads = 2 if RPI_AVAILABLE else num_threads

        # Initialize the TFLite interpreter with appropriate settings for Raspberry Pi
        self.interpreter = tf.lite.Interpreter(
            model_path=model_path,
            num_threads=threads,
            experimental_delegates=None  # No need for special delegates on RPi
        )
...
class GestureRecognizer:
    ...
    def __init__(self, model_type='lstm'):
        # Paths for the sequence model - try .keras extension first, fallback to .h5
        self.model_type = model_type
        keras_path = os.path.join(TRAINED_MODEL_DIR, f'gesture_model_{model_type}_final.keras')
        h5_path = os.path.join(TRAINED_MODEL_DIR, f'gesture_model_{model_type}_final.h5')
    ...
    def _load_model(self):
        ...
        try:
            # On Raspberry Pi, we should try to use TFLite instead of full model if possible
            if RPI_AVAILABLE and self.model_path.endswith('.h5'):
                print("Using TensorFlow Lite for model loading on Raspberry Pi")
                converter = tf.lite.TFLiteConverter.from_keras_model(tf.keras.models.load_model(self.model_path))
                tflite_model = converter.convert()

                # Save the TFLite model temporarily
                tflite_path = os.path.join(TRAINED_MODEL_DIR, f'gesture_model_{self.model_type}_temp.tflite')
                with open(tflite_path, 'wb') as f:
                    f.write(tflite_model)

                # Load the TFLite model
                self.interpreter = tf.lite.Interpreter(model_path=tflite_path, num_threads=2)
                self.interpreter.allocate_tensors()
                self.input_details = self.interpreter.get_input_details()
                self.output_details = self.interpreter.get_output_details()
    ...
    def update(self, handshape, orientation, movement, confidence, is_stationary):
        ...
        if self.state == "WAITING":
            ...
            if is_stationary:
                ...
                # Add debug output every second frame
                if self.stationary_counter % 2 == 0:
                    print(f"Stationary frame {self.stationary_counter}/{STATIONARY_THRESHOLD}")
                ...
                # Only reset if we were building up the counter
                if self.stationary_counter > 0:
                    print(f"Movement detected - resetting stationary counter ({self.stationary_counter} → 0)")
                self.stationary_counter = 0
    ...
    def _process_sequence(self):
        ...
        try:
            ...
            # Make prediction - check if we're using TFLite or full model
    ...
    def _preprocess_sequence(self, sequence):
        ...
        return np.array([sequence_features], dtype=np.float32)  # Explicitly set dtype for TFLite
...
def get_args():
    ...
    parser.add_argument("--width", help='cap width', type=int, default=640)  # Reduced from 960 for RPi
    parser.add_argument("--height", help='cap height', type=int, default=480)  # Reduced from 540 for RPi
    parser.add_argument("--model_selection", type=int, default=0)  # Lower complexity for RPi (0 instead of 1)
    parser.add_argument('--use_static_image_mode', action='store_true')
    parser.add_argument("--min_detection_confidence", type=float, default=0.7)
    parser.add_argument("--min_tracking_confidence", type=int, default=0.5)
    parser.add_argument("--max_hands", type=int, default=1)  # Reduced to 1 hand for performance
    ...
    parser.add_argument("--rpi_camera", action="store_true",
                        help="Use Raspberry Pi camera module instead of USB camera")
...
def detect_simplified_movement(position_history,
                               min_movement=2.5):  # Reduced from 5.0 to 2.5 for better stationary detection
    """
...
def draw_sentence_display(image, recognizer, start_y=150):
    ...
    bg_width = min(900, image.shape[1] - 20)  # Make sure it fits on screen
    bg_height = 120
    ...
    if completion_status > 0:
        ...
        # Label
        cv.putText(image,
                   f"Auto-completing in {NO_HANDS_TIMEOUT - completion_status * NO_HANDS_TIMEOUT:.1f}s...",
                   (bar_x + bar_width // 2 - 150, bar_y - 5),
                   cv.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 255), 1, cv.LINE_AA)

    # Draw controls
    controls_text = "SPACE: Complete | BACKSPACE: Clear | S: Speak"
...
def draw_help(image, start_y=450):
    """Draw help text"""
    # Background
    bg_width = min(300, image.shape[1] - 20)  # Make sure it fits on screen
    cv.rectangle(image, (10, start_y), (10 + bg_width, start_y + 100), (40, 40, 40), -1)
...
def setup_rpi_camera():
    """Setup Raspberry Pi camera if available and requested"""
    try:
...
def main():
    ...
    # Setup camera (standard or Raspberry Pi camera)
    rpi_camera = None
    if args.rpi_camera and RPI_AVAILABLE:
        rpi_camera = setup_rpi_camera()
    ...
    if rpi_camera is None:
        ...
        cap.set(cv.CAP_PROP_FPS, 30)  # Try to set 30 FPS - may not be supported

        # Check if camera opened successfully
        if not cap.isOpened():
            print("Error: Could not open camera. Check device number.")
            return

    # Initialize MediaPipe Hands with performance parameters for Raspberry Pi
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(
        model_complexity=args.model_selection,  # Lower complexity for RPi
    ...
    try:
        while True:
            # Frame rate throttling for Raspberry Pi (process every other frame)
            frame_count += 1
            process_this_frame = (not RPI_AVAILABLE) or (frame_count % 2 == 0)
            ...
            # Only process every other frame if on RPi for performance
